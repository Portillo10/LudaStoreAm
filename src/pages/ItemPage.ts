...
export class ItemPage extends BasePage {
  async getTitle() {
    const title = await (
      await this.selectOne(["#productTitle"])
    )?.textContent();
    if (title) {
      return title.trim();
    } else {
    ...
    for (const selector of detailSelectors) {
      ...
      if (detailsElements && elementsCount && elementsCount > 0) {
        ...
        for (let i = 0; i < elementsCount; i++) {
          ...
          //   .textContent();
          const key = await (
            await element.$(selector.keySelector)
          )?.textContent();
          const value = await (
            await element.$(selector.valueSelector)
          )?.textContent();
  ...
  async getSpecs() {
    ...
    if (!specsElements) throw new Error("Especificaciones no disponibles");
    const specs = await this.convertElementsToDict(specsElements);
    if (!specs) throw new Error("Error extrayendo especificaciones");
    return specs;
  }
  ...
  async getDescription() {
    ...
    if (descriptionElements && descriptionElements?.length > 0) {
      return (
        await Promise.all(
          descriptionElements.map(async (element) => {
  ...
  async getImgSources() {
    const imgAlts = this.openPage?.locator(
      "#altImages li.imageThumbnail input"
    );
    ...
    const imgs = await this.openPage?.$$eval(
      "div.imgTagWrapper img.a-dynamic-image",
      (elements) => {
        return elements.map((element) => element.getAttribute("src"));
      }
    );
    ...
    const imgSources = (
      await Promise.all(
        imgs.map(async (img) => {
          ...
          if (await allowImageSize(newSrc)) {
  ...
  async getWeight() {
    const productOverView = await this.openPage?.$(
    ...
    if (!text) throw new Error("No es posible extraer el peso");
